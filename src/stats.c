...
int main (){
	...
	display_window* quit_window = display_create_window(EXIT, false, "w1/3:h1/3:w1/3:5");
	display_set_content_node_alignment(display_window_add_content_node(quit_window, UNKNOWN, " Are you sure you want to quit? Select option or press 'q' to confirm. "), CENTER);
	//display_set_window_expansion(quit_window, true);
	display_window_box(quit_window, '-', '|');

	display_window* yes_window = display_create_window(EXIT, true, "w1/2-4:h1/3+3:3:1");
	...
	display_window* no_window = display_create_window(EXIT, true, "w1/2+1:h1/3+3:2:1");
